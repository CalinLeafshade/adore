local actor = adore.class("adore.actor")
actor:include(adore.persist.mixin.persistable)

function actor:initialize(name)
	self.name = name
	self.position = adore.vector(200,400)
	...
	self.speechColor = adore.graphics.colour(200,200,200)
	self.speechTimer = 0
	self.speaking = false

	-- All actors have an inventory
	self.inventory = adore.inventories.new("actor_" .. name)

	-- All actors are persisted
	adore.persist.register("actor_" .. self.name, self)
...
function actor:setScene(scene, x,y)
	...
	if x then
		...
		self.position = adore.vector(x,y)
	end
...
function actor:face(x,y)
	if type(x) == "string" then
		...
		local dir = adore.util.directionFromVector(adore.vector(x,y) - self.position)
...
local function directionFromVector(v)
...
--		self.direction = directionFromVector(moveVector) or self.direction
...
function actor:hitTest(x,y)
	if not adore.util.contains(x,y, self:bbox()) then
		return false
	end
	local ani = self:getAnimation()
	if ani then
